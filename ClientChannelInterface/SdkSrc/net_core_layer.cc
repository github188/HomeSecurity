...
CNetCoreLayer::~CNetCoreLayer(void)
{
	...
	{
		LOG4CXX_ERROR(g_logger_sdk, "CNetCoreLayer::~CNetCoreLayer:event_base_loopexit error.");
	}
...
bool CNetCoreLayer::InitNetCore(CNetInterLayer* pNetInterLayer, const std::string& ip, const int port)
{
	...
	LOG4CXX_TRACE(g_logger_sdk, "CNetCoreLayer::InitNetCore:libevent version = " << libevent_version);
	...
	if (0 != ret)
	{
		LOG4CXX_ERROR(g_logger_sdk, "CNetCoreLayer::InitNetCore:pipe errorCode = " << ret);
		return false;
	}

	server_ip_ 	= ip;
	port_		= port;
	LOG4CXX_TRACE(g_logger_sdk, "CNetCoreLayer::InitNetCore server_ip = "<< server_ip_ << ", server_port = " << port_);
...
void CNetCoreLayer::Run()
{
	...
	LOG4CXX_INFO(g_logger_sdk, "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^CNetCoreLayer::Run....^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^");

	int i = event_base_dispatch(base_);

	LOG4CXX_INFO(g_logger_sdk, "CNetCoreLayer::event_base_dispatch = " << i);
}
...
void CNetCoreLayer::DoLocalRead(evutil_socket_t local_tcp_server, short event, void *arg)
{
	...
	if (size != 1)
	{
		LOG4CXX_ERROR(g_logger_sdk, "CNetCoreLayer::DoLocalRead recv error. error = " << evutil_socket_error_to_string(EVUTIL_SOCKET_ERROR()));
		return;
	}
	...
	{
		LOG4CXX_WARN(g_logger_sdk, "CNetCoreLayer::DoLocalRead pop_front NULL.");
		return;
	}
...
void CNetCoreLayer::ShortConnectionOpt(const REQUEST_INFO& requestInfo)
{
	...
	{
		LOG4CXX_WARN(g_logger_sdk, "CNetCoreLayer::ShortConnectionOpt:IN_DATA not enough memory!");
		return;
	}
	...
	{
		LOG4CXX_ERROR(g_logger_sdk, "CNetCoreLayer::ShortConnectionOpt:bufferevent_socket_new");
	...
	//
	LOG4CXX_TRACE(g_logger_sdk, "CNetCoreLayer::ShortConnectionOpt request_append=" << request_append);
	//
	int send_size = send(requestInfo.sfd, request_append.c_str(), request_append.length(), 0);
	if (send_size < 0)
	{
		LOG4CXX_ERROR(g_logger_sdk, "CNetCoreLayer::AddShortConnectionResquest:send failed. error = " << evutil_socket_error_to_string(EVUTIL_SOCKET_ERROR()));
		close(requestInfo.sfd);
		return;
	}
}
...
void CNetCoreLayer::PersistConnectionOpt(const REQUEST_INFO& requestInfo)
{
	...
	{
		LOG4CXX_FATAL(g_logger_sdk, "CNetCoreLayer::PersistConnectionOpt!!!!!!");
		return;
	}
...
int CNetCoreLayer::AddShortConnectionResquest(const std::string& request)
{
	...
	{
		LOG4CXX_ERROR(g_logger_sdk, "CNetCoreLayer::AddShortConnectionResquest:setsockopt(TCP_NODELAY) short_sfd failed. error = "
...
int CNetCoreLayer::AddPersistConnectionRequest(const std::string& request)
{
	...
	if (!persist_connection_has_)
	{
		...
		{
			LOG4CXX_ERROR(g_logger_sdk, "CNetCoreLayer::AddPersistConnectioRequest:setsockopt(TCP_NODELAY)  \
		...
		if (size < 0)
		{
			LOG4CXX_ERROR(g_logger_sdk, "CNetCoreLayer::AddPersistConnectioRequest:send error. errorCode = " << evutil_socket_error_to_string(EVUTIL_SOCKET_ERROR()));
			close(persist_sfd_);
			return CONN_OTHER_ERROR;
		}

		LOG4CXX_INFO(g_logger_sdk, "CNetCoreLayer::AddPersistConnectioRequest:persist_connection_has_ = " << persist_connection_has_);
...
void CNetCoreLayer::DoRemoteShortTcpError(struct bufferevent *bev, short event, void *arg)
{
	...
	LOG4CXX_INFO(g_logger_sdk, "CNetCoreLayer::DoRemoteShortTcpError:fd = " << socket_fd);

	if (event & BEV_EVENT_TIMEOUT)
	{
		LOG4CXX_WARN(g_logger_sdk, "CNetCoreLayer::DoRemoteShortTcpError:TimeOut.");
	}
	else if (event & BEV_EVENT_EOF)
	{
		LOG4CXX_WARN(g_logger_sdk, "CNetCoreLayer::DoRemoteShortTcpError:connection closed.");
	}
	else if (event & BEV_EVENT_ERROR)
	{
		LOG4CXX_ERROR(g_logger_sdk, "CNetCoreLayer::DoRemoteShortTcpError:some other error = " << evutil_socket_error_to_string(EVUTIL_SOCKET_ERROR()));
	}
...
void CNetCoreLayer::DoRemotePersistTcpError(struct bufferevent *bev, short event, void *arg)
{
	...
	LOG4CXX_INFO(g_logger_sdk, "CNetCoreLayer::DoRemotePersistTcpError:fd = " << socket_fd);

	if (event & BEV_EVENT_EOF)
	{
		pThis->pNetInterLayer_->ReciveData(std::string(STR_PTCP_HAS_CLOSED), PERSIST_CONNECTION);
		LOG4CXX_WARN(g_logger_sdk, "CNetCoreLayer::DoRemotePersistTcpError:connection closed.");
	}
	else if (event & BEV_EVENT_ERROR)
	{
		pThis->pNetInterLayer_->ReciveData(std::string(STR_PTCP_HAS_ERROR), PERSIST_CONNECTION);
		LOG4CXX_ERROR(g_logger_sdk, "CNetCoreLayer::DoRemotePersistTcpError:some other error = " << evutil_socket_error_to_string(EVUTIL_SOCKET_ERROR()));
	}
...
int CNetCoreLayer::ClosePersistConnection()
{
	...
	LOG4CXX_TRACE(g_logger_sdk, "*********************ClosePersistConnection:persist_sfd_ = " << persist_sfd_ << "*********************");
	if (ret != 0)
	{
		LOG4CXX_ERROR(g_logger_sdk, " CNetCoreLayer::ClosePersistConnection error = " << evutil_socket_error_to_string(EVUTIL_SOCKET_ERROR()));
	}
	return ret;
}
...
int CNetCoreLayer::connect_nonb( int sockfd, struct sockaddr* saptr, socklen_t salen, int nsec )
{
	...
	if ((n = ::connect(sockfd, saptr, salen) < 0))
	{
		...
		{
			LOG4CXX_ERROR(g_logger_sdk, "CNetCoreLayer::connect_nonb:connect failed. errorcode = " << errno);
			return (-1);
		}
	}
	...
	if ((n = select(sockfd+1, &rset, &wset, NULL, nsec?&tval:NULL)) == 0)
	{
		close(sockfd);
		errno = ETIMEDOUT;
		LOG4CXX_ERROR(g_logger_sdk, "CNetCoreLayer::connect_nonb:select failed. errorcode = " << errno);
		return -1;
	}
	...
	if (FD_ISSET(sockfd, &rset) || FD_ISSET(sockfd, &wset))
	{
		...
		{
			LOG4CXX_ERROR(g_logger_sdk, "CNetCoreLayer::connect_nonb:getsockopt failed. errorcode = " << errno);
			return -1;
		}
	}
	else
		LOG4CXX_FATAL(g_logger_sdk, "CNetCoreLayer::connect_nonb:select error: sockfd not set");
	...
	if (error)
	{
		close(sockfd);
		errno = error;
		LOG4CXX_ERROR(g_logger_sdk, "CNetCoreLayer::connect_nonb:ioctlsocket1 failed. errorcode = " << errno);
		return -1;
	}
...
bool CNetCoreLayer::SetTCP_NODELAY(evutil_socket_t sfd)
{
	...
	if (-1 == nErr)
	{
		LOG4CXX_ERROR(g_logger_sdk, "CNetCoreLayer::connect_nonb:SetTCP_NODELAY failed. error = " << strerror(errno));
		return false;
	}
...
bool CNetCoreLayer::GetIpByDomain(const std::string& domain, std::string& ip)
{
	...
	if (NULL == st_host)
	{
		LOG4CXX_ERROR(g_logger_sdk, "CNetCoreLayer::GetIpByDomain failed.domain = " << domain
