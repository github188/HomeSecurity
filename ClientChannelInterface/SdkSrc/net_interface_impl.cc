...
bool CClientNetInterfaceImpl::Init(const std::string& ip,const int port)
{
	...
	return true;
		...
		bool CClientNetInterfaceImpl::Init(const std::string& ip,const int port, const std::string& log_path)
		{
			...
			g_logger_sdk = Logger::getLogger("NetInterface");
...
int CClientNetInterfaceImpl::GetLiveStatus()
{
	...
	LOG4CXX_TRACE(g_logger_sdk, "--------CUserInterfaceImpl::GetLiveStatus-------- request = " << request);
	...
	LOG4CXX_TRACE(g_logger_sdk, "********CUserInterfaceImpl::GetLiveStatus******** response = " << response);
	...
	{
		LOG4CXX_WARN(g_logger_sdk, "CUserInterfaceImpl::GetFriendsStatus:GetLiveStatus invalid : " << response);
		ret = JSON_INVALID;
		return ret;
	}
}
...
void CClientNetInterfaceImpl::ServerPushMessageOpt( const std::string& server_push_message)
{
	...
	LOG4CXX_TRACE(g_logger_sdk, "CUserInterfaceImpl::ServerPushMessageOpt : " << server_push_message);
...
int CClientNetInterfaceImpl::GetResponseByRequestPersistentConnection(const std::string& request, std::string& response)
{
	...
	if(!pNetDataOpt_->RestructJsonByMessageId(request, message_id, new_request))
	{
		LOG4CXX_ERROR(g_logger_sdk, "CUserInterfaceImpl::GetResponseByRequestPersistentConnection:RestructJsonByMessageId error.");
		return JSON_PARSE_ERROR;
	}

	LOG4CXX_TRACE(g_logger_sdk, "--------CUserInterfaceImpl::GetResponseByRequestPersistentConnection-------- new_request = " << new_request);

	int ret = pInterLayer_->GetResponseByRequest(message_id, PERSIST_CONNECTION, new_request, response);
	if (ret != SUCCESS)
	{
		return ret;
	}

	LOG4CXX_TRACE(g_logger_sdk, "********CUserInterfaceImpl::GetResponseByRequestPersistentConnection******** response = " << response);
...
int CClientNetInterfaceImpl::GetResponseByRequestShortConnection(const std::string& request, std::string& response)
{
	...
	if(!pNetDataOpt_->RestructJsonByMessageId(request, message_id, new_request))
	{
		LOG4CXX_ERROR(g_logger_sdk, "CUserInterfaceImpl::GetResponseByRequestShortConnection:RestructJsonByMessageId error.");
		return JSON_PARSE_ERROR;
	}

	LOG4CXX_TRACE(g_logger_sdk, "--------CUserInterfaceImpl::GetResponseByRequestShortConnection-------- new_request = " << new_request);

	int ret = pInterLayer_->GetResponseByRequest(message_id, SHORT_CONNECTION, new_request, response);
	if (ret != SUCCESS)
	{
		return ret;
	}

	LOG4CXX_TRACE(g_logger_sdk, "********CUserInterfaceImpl::GetResponseByRequestShortConnection******** response = " << response);
