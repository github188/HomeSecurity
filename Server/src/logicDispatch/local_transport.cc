...
std::map<int, int> 	CLocalTransport::map_logic_sfd_;
...
void CLocalTransport::AcceptCb(evutil_socket_t listen_socket, short event, void* arg)
{
	...
	/* 利用逻辑处理进程绑定的端口号将相应的逻辑处理进程和sfd对应 */
	int logic_pro_port = ntohs(sin.sin_port);
	int module = 0;
	if (utils::G<CGlobalSettings>().accountLogic_port_ == logic_pro_port)
	{
		LOG4CXX_INFO(g_logger, "CLocalTransport::AcceptCb:clientLogin connection... sfd = " << sfd);
		module = ACCOUNT_BUSINESS_PROCESS;
	}
	else if (utils::G<CGlobalSettings>().deviceInfo_port_  == logic_pro_port)
	{
		LOG4CXX_INFO(g_logger, "CLocalTransport::AcceptCb:deviceInfo connection...sfd = " << sfd);
		module = DEV_INFO_PRO;
	}
	else if (utils::G<CGlobalSettings>().alarmLogic_port == logic_pro_port)
	{
		LOG4CXX_INFO(g_logger, "CLocalTransport::AcceptCb:alarmLogic connection...sfd = " << sfd);
		module = ALARM_PROCESS;
	}
	else if (utils::G<CGlobalSettings>().deviceInfo_HomeCloud_port_ == logic_pro_port)
	{
		LOG4CXX_INFO(g_logger, "CLocalTransport::AcceptCb:deviceInfo_HomeCloud connection...sfd = " << sfd);
		module = DEV_INFO_HOMECLOUD;
	}
	else
	{
		LOG4CXX_WARN(g_logger, "CLocalTransport::AcceptCb:Unknow connection.");
		return;
	}
	pThis->map_logic_sfd_.insert(std::pair<int, int>(module, sfd));
...
void CLocalTransport::ReadCb(struct bufferevent *bev, void *arg)
{
	...
	if (utils::FindCRLF(str_recv))
	{
		...
		{
			SendToClient(vec_str.at(i).c_str());
		}
...
void CLocalTransport::SendToClient(const char* buf)
{
	CJsonOpt jsonOpt;
	std::string string_to_client;
	int sfd = 0;
	int sfd_id = 0;
	if (!jsonOpt.RestructJsonStringToclient(buf, string_to_client, sfd, sfd_id))
	{
		LOG4CXX_WARN(g_logger, "CLocalTransport::SendToClient:RestructJsonStringToclient error. message = " << buf);
		return;
	}
...
bool CLocalTransport::SendToLogicModule(const char* message, const int module_type)
{
	...
	std::map<int, int>::iterator iter;
	iter = map_logic_sfd_.find(module_type);
	if (iter == map_logic_sfd_.end())
	{
	...
	LOG4CXX_TRACE(g_logger, "CLocalTransport::SendToLogicModule:send data = " << message);
