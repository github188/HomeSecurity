...
bool CJsonOpt::JsonParseDeviceRegister(DEVICE_INFO &device_info)
{
	if(VerifyJsonField(in_, JK_DEVICE_GUID) && VerifyJsonField(in_, JK_DEVICE_NAME) && VerifyJsonField(in_, JK_DEVICE_TYPE))
	{
		...
		if (VerifyJsonField(in_, JK_DEVICE_HUMITURE_FLAG))
		{
			device_info.humiture_flag = in_[JK_DEVICE_HUMITURE_FLAG].as_int();
		}
		if (VerifyJsonField(in_, JK_DEVICE_RESET_FLAG))
		{
			device_info.reset_flag = in_[JK_DEVICE_RESET_FLAG].as_int();
		}
...
std::string CJsonOpt::JsonJoinGetUserDevices(const int result, const USER_DEVICES_LIST& user_devices_list)
{
	...
	std::vector<USER_DEVICE_INFO>::const_iterator iter;
	for (iter = user_devices_list.vec_user_device_info.begin(); iter != user_devices_list.vec_user_device_info.end(); ++iter)
	{
		JSONNode device_json;
		device_json.push_back(JSONNode(JK_DEVICE_GUID, (*iter).device_info.device_guid));
		device_json.push_back(JSONNode(JK_DEVICE_TYPE, (*iter).device_info.device_type));
		...
		device_json.push_back(JSONNode(JK_DEVICE_NAME, (*iter).device_info.device_name));
		...
		device_json.push_back(JSONNode(JK_DEVICES_ONLINE_STATUS, (*iter).device_info.online_status));
		...
		device_json.push_back(JSONNode(JK_VIDEO_LINK_TYPE, (*iter).user_device_config.video_link_type));
		device_json.push_back(JSONNode(JK_DEVICE_VIDEO_USERNAME, (*iter).user_device_config.video_username));
		device_json.push_back(JSONNode(JK_DEVICE_VIDEO_PASSWORD, (*iter).user_device_config.video_password));
		device_json.push_back(JSONNode(JK_DEVICE_VIDEO_IP, (*iter).user_device_config.video_ip));
		device_json.push_back(JSONNode(JK_DEVICE_VIDEO_PORT, (*iter).user_device_config.video_port));
...
std::string CJsonOpt::JsonJoinGetDeviceUsers(const int result, const std::set<std::string>& set_un, const std::string& device_name, const std::string& alarm_time, const int full_alarm_mode)
{
	...
	for(it = set_un.begin(); it != set_un.end(); ++it)
	{
		if (*it != std::string(""))
...
std::string CJsonOpt::JsonJoinGetDeviceInfo(const int result, const DEVICE_INFO& device_info)
{
	...
	json_deviceinfo.push_back(JSONNode(JK_DEVICE_RESET_FLAG, device_info.reset_flag));
...
std::string CJsonOpt::JsonJoinGetUserDeviceInfo(const int result, const USER_DEVICE_INFO& user_device_info)
{
	...
	json_deviceinfo.push_back(JSONNode(JK_DEVICE_NAME, user_device_info.device_info.device_name));
...
std::string CJsonOpt::JsonJoinGetUserDevicesStatusInfo(const int result, const DEVICES_LIST& devicesList)
{
	...
	char timestamp[32] = {0};
	std::vector<DEVICE_INFO>::const_iterator iter;
	for (iter = devicesList.vec_device_info.begin(); iter != devicesList.vec_device_info.end(); ++iter)
	{
		memset(timestamp, 0, sizeof (timestamp));
		strftime(timestamp, sizeof (timestamp), "%Y-%m-%d %H:%M:%S", &(*iter).timestamp);

		JSONNode device_json;
		device_json.push_back(JSONNode(JK_DEVICE_GUID, (*iter).device_guid));
		device_json.push_back(JSONNode(JK_DEVICE_NAME, (*iter).device_name));
		...
		device_json.push_back(JSONNode(JK_DEVICES_ONLINE_STATUS, (*iter).online_status));
...
bool CJsonOpt::JsonParseAddDeviceChannel(std::string& device_guid, int& add_sum)
{
	if (!(VerifyJsonField(in_, JK_DEVICE_GUID) && VerifyJsonField(in_, JK_DEVICE_CHANNEL_SUM)))
		return false;
...
{
	JsonJoinCommon(MODIFY_DEVICE_CHANNEL_NAME, result);
	return out_.write();
}

bool CJsonOpt::JsonParseGetDeviceRelationNum(std::string& device_guid)
{
	if (!VerifyJsonField(in_, JK_DEVICE_GUID))
		return false;
...
std::string CJsonOpt::JsonJoinGetDeviceRelationNum(const int result, const int relation_num)
{
	JsonJoinCommon(GET_DEVICE_RELATION_NUM_RESPONSE, result);
	out_.push_back(JSONNode(JK_DEVICE_RELATION_NUM, relation_num));
	return out_.write();
}

bool CJsonOpt::JsonParseSetApConfFlag(std::string& device_guid)
{
	if (!VerifyJsonField(in_, JK_DEVICE_GUID))
		return false;
...
std::string CJsonOpt::JsonJoinSetApConfFlag(const int result)
{
	JsonJoinCommon(SET_AP_CONF_FLAG_RESPONSE, result);
	return out_.write();
}

bool CJsonOpt::JsonParseGetDeviceOnlineState(std::string& device_guid)
{
	if (!VerifyJsonField(in_, JK_DEVICE_GUID))
		return false;
...
std::string CJsonOpt::JsonJoinGetDeviceOnlineState(const int result, const int online_flag)
{
	JsonJoinCommon(GET_DEVICE_ONLINE_STATE_RESPONSE, result);
	out_.push_back(JSONNode(JK_DEVICES_ONLINE_STATUS, online_flag));
	return out_.write();
}

bool CJsonOpt::JsonParseCheckDeviceBindState(std::string& device_guid)
{
	if (!VerifyJsonField(in_, JK_DEVICE_GUID))
		return false;
...
std::string CJsonOpt::JsonJoinCheckDeviceBindState(const int result, const int online_flag, const int relation_num)
{
	JsonJoinCommon(CHECK_DEVICE_BIND_STATE_RESPONSE, result);
	out_.push_back(JSONNode(JK_DEVICES_ONLINE_STATUS, online_flag));
	out_.push_back(JSONNode(JK_DEVICE_RELATION_NUM, relation_num));
	return out_.write();
}

bool CJsonOpt::JsonParseReportDeviceCloudSeeOnline(std::string& device_guid)
{
	if (!VerifyJsonField(in_, JK_DEVICE_GUID))
		return false;
...
std::string CJsonOpt::JsonJoinReportDeviceCloudSeeOnline(const int result)
{
	JsonJoinCommon(REPORT_DEVICE_CLOUDSEE_ONLINE_RESPONSE, result);
	return out_.write();
}

bool CJsonOpt::JsonParseReportDeviceReset(std::string& device_guid)
{
	if (!VerifyJsonField(in_, JK_DEVICE_GUID))
		return false;
...
std::string CJsonOpt::JsonJoinReportDeviceReset(const int result)
{
	JsonJoinCommon(REPORT_DEVICE_RESET_RESPONSE, result);
	return out_.write();
}
