...
public:
	...
	int GetUserDevicesStatusInfoFromDB(const std::string& username, std::vector<std::string>& vec_device, DEVICES_LIST& deviceList);
	...
	int RemoveBindByDeviceHoldUserToDB(const std::string& device_guid, const std::string& hold_user);
	...
	int GetDeviceUsersFromDB(const std::string& device_guid, std::set<std::string> &set_un, std::string &device_name, std::string &alarm_time, int &full_alarm_mode, const std::string& device_username, const std::string& device_password, const int reset_flag);
	int VerifyDeviceExistFromDB(const std::string& device_guid);
	int AddDeviceChannelToDB(const std::string& device_guid, const int add_sum);
	int DeleteDeviceChannelToDB(const std::string& device_guid, const int channel_no);
	int GetDeviceChannelFromDB(const std::string& device_guid, std::vector<DEVICE_CHANNEL_INFO>& vec_device_channel);
	int ModifyDeviceChannelNameToDB(const std::string& device_guid, const int& channel_no, const std::string& channel_name);
	int GetDeviceRelationNumFromDB(const std::string& device_guid, int& relation_num);
	int SetApConfFlagToDB(const std::string& device_guid);
