...
enum ConditionType
{
	CT_EQUAL = 1, CT_EQUAL_STRING, CT_NOT_EQUAL, CT_ABOVE, CT_BELOW, CT_FUZZY,CT_FUZZY_PRE,CT_AREA, CT_GROUP,
};
...
string AssembleCondition(const map<string, SelCondition>& mapConditions)
{
	...
	for (CI p = mapConditions.begin(); p != mapConditions.end(); ++p)
	{
		...
		case CT_FUZZY:
			...
			break;
		case CT_FUZZY_PRE:
			strCmd = string(" AND ") + colName + string(" like ") + p->second.m_strValueFirst;
			break;
		case CT_AREA:
