...
int main(int argc, char **argv)
{
	/* process arguments */
	int c;
	std::string version = std::string("1.0.196:385");
...
void Run()
{
	...
	if(!CCloudSeeTransport::GetInstance()->SetupChannelWithCloudSeeServer())
	{
		LOG4CXX_FATAL(g_logger, "SetupChannelWithCloudSeeServer failed.");
		exit(EXIT_FAILURE);
	}
...
void InitConfigure()
{
	...
	initConfObj.InitLog4cxx("deviceInfo");
	if (!initConfObj.LoadConfiguration("deviceInfo"))
	{
...
void InitSql()
{
	...
	sqlConnInfo.password = utils::G<ConfigFile>().read<std::string> ("sqldb.password", "123456");
...
void SettingsAndPrint()
{
	...
	utils::G<CGlobalSettings>().bind_port_		= utils::G<ConfigFile>().read<int> ("deviceInfo.bind.port", 5657);
	utils::G<CGlobalSettings>().thread_num_		= utils::G<ConfigFile>().read<int> ("deviceInfo.worker.thread.num", 4);
	utils::G<CGlobalSettings>().queue_num_		= utils::G<ConfigFile>().read<int> ("deviceInfo.worker.queue.num", 16);
	utils::G<CGlobalSettings>().stats_listen_port_ = utils::G<ConfigFile>().read<int>("deviceInfo.stats.listen.port", 8023);
	utils::G<CGlobalSettings>().cloudsee_ip_	= utils::G<ConfigFile>().read<std::string> ("cloudsee.proc.ip","58.56.109.18");
	utils::G<CGlobalSettings>().cloudsee_port_	= utils::G<ConfigFile>().read<int> ("cloudsee.proc.port", 8634);
	...
	LOG4CXX_INFO(g_logger, "******cloudsee.proc.ip =" 		<< utils::G<CGlobalSettings>().cloudsee_ip_  << "******");
	LOG4CXX_INFO(g_logger, "******cloudsee.proc.port ="<< utils::G<CGlobalSettings>().cloudsee_port_<< "******");
}
