...
boost::mutex CLocalTransport::instance_mutex_;
...
bool CLocalTransport::SetupChannelWithRelayServer()
{
	main_base_	= NULL;
	sfd_		= 0;
...
void *CLocalTransport::SetupThread(void *arg)
{
	CLocalTransport* pThis = static_cast<CLocalTransport*>(arg);

	/* 当和转发进程的连接未正常建立时定时进行重试 */
	while (!pThis->local_connect_flag_)
	{
		...
		LOG4CXX_INFO(g_logger, "SetupChannelWithRelayServer done.");
...
void CLocalTransport::CreateThread(void *(*func)(void *), void *arg)
{
	pthread_t thread;
	pthread_attr_t attr;
	int ret;
	...
	{
		LOG4CXX_FATAL(g_logger, "CLocalTransport::CreateThread:Can't create thread:" << strerror(ret));
		exit(1);
	}
}
...
bool CLocalTransport::InitSocketFd(evutil_socket_t& sfd)
{
	...
	if (sfd < 0)
	{
		LOG4CXX_ERROR(g_logger, "CLocalTransport::InitSocketFd:SocketFd error = " << strerror(errno));
		return false;
	}
	...
	servaddr.sin_addr.s_addr = inet_addr((utils::G<CGlobalSettings>().relay_server_ip_).c_str());
	servaddr.sin_port = htons(utils::G<CGlobalSettings>().relay_server_port_);
...
void CLocalTransport::ReadCb(struct bufferevent *bev, void *arg)
{
	...
	if (utils::FindCRLF(str_recv))
	{
		...
		{
			CRelayMessageOpt relayMessageOpt(sfd);
			relayMessageOpt.StartRelayMessageOpt(vec_str.at(i));
		}
...
bool CLocalTransport::SendToRelay(std::string& message)
{
	boost::mutex::scoped_lock Lock(mutex_);
	message.append(CRLF);
	if (message.length() >= DATA_BUFFER_SIZE)
	{
		LOG4CXX_WARN(g_logger, "CLogicOperate::SendToRelay data too large. data = " << message);
		return false;
	}
	return SocketOperate::WriteSfd(sfd_, message.c_str(), message.length());
}
