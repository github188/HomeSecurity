...
bool CMasterThread::InitMasterThread()
{
	...
	{
		LOG4CXX_ERROR(g_logger, "CMasterThread::InitNetCore The libevent version Require at  2.0.*");
		return false;
	}
	...
	if(event_add(listen_event_, NULL) == -1)
	{
		...
		LOG4CXX_ERROR(g_logger, "CMasterThread::InitNetCore:event_add errorCode = " << error_code
