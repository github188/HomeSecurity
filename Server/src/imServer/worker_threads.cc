...
void CWorkerThread::ClientTcpErrorCb(struct bufferevent *bev, short event, void *arg)
{
	...
	{
		LOG4CXX_WARN(g_logger, "CWorkerThread::ClientTcpErrorCb:TimeOut. username = " << c->chUsername);
	}
...
void CWorkerThread::conn_close(conn *c, struct bufferevent *bev)
{
	...
	LOG4CXX_TRACE(g_logger, "CWorkerThread::conn_close sfd = " << c->sfd << ", username = " << c->chUsername);
