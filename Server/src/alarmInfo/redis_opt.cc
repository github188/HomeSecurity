...
bool CRedisOpt::LRem(const std::string& key, const std::string& val){
	bool bret = false;
	if(val.empty()){
		return bret;
	}
	std::string command = "LREM " + key + " 0 " + val;
	reply_ = (redisReply*) redisCommand(conn_, command.c_str());
	if(reply_ == NULL){
		LOG4CXX_ERROR(g_logger, "redisCommand: error = " << conn_->errstr);
		SafeFreeRedisContext(conn_);
	...
	if (REDIS_REPLY_INTEGER != reply_->type){
		LOG4CXX_ERROR(g_logger, "CRedisOpt::LREM error. command = " << command << ", errorMessage = " << reply_->str);
	}
	else{
		bret = true;
	}
...
bool CRedisOpt::LRange(const std::string& key, const int idx_start, const int idx_stop, std::vector<std::string>& vec_value)
{
	bool bret = false;

	reply_ = (redisReply*) redisCommand(conn_, "LRANGE %s %d %d", key.c_str(), idx_start, idx_stop);
	if (NULL == reply_)
	{
		LOG4CXX_ERROR(g_logger, "redisCommand: error = " << conn_->errstr);
		SafeFreeRedisContext(conn_);
...
bool CRedisOpt::Hget(const std::string& key, const std::string& field, std::string& value)
{
	...
	else if (REDIS_REPLY_ERROR == reply_->type)
	{
...
bool CRedisOpt::Keys(const std::string& keys_pattern, std::vector<std::string>& vec_keys)
{
	bool bret = false;

	reply_ = (redisReply*) redisCommand(conn_, "KEYS %s", keys_pattern.c_str());
	if (NULL == reply_)
	{
		LOG4CXX_ERROR(g_logger, "redisCommand::KEYS error = " << conn_->errstr);
		SafeFreeRedisContext(conn_);
		return false;
	}
	...
	{
		SafeFreeReplyObject(reply_);
		LOG4CXX_ERROR(g_logger, "CRedisOpt::KEYS error. key = " << keys_pattern << ", errorMessage = " << reply_->str);
		return bret;
	}
	...
	for (int i = 0; i < len; ++i)
	{
		vec_keys.push_back(reply_->element[i]->str);
	}
